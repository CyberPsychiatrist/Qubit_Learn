...
class SupaDB:
    ...
    def get_donation_by_id(self, donation_id: str) -> Optional[Dict[str, Any]]:
        res = self.client.table("donations").select("*").eq("id", donation_id).single().execute()
        return res.data if res and res.data else None

    def get_donations(self, email: str) -> List[Dict[str, Any]]:
        res = self.client.table("donations").select("*").eq("email", email).order("created_at", desc=True).execute()
        return res.data or []
